...
Looking to mix up a backend with express/sequelize and a frontend with react/redux? That's `Room8`!
...
```
git remote add Room8 https://github.com/FullstackAcademy/Room8.git
git fetch Room8
git merge Room8/master
```

Why did we do that? Because every once in a while, `Room8` may be updated with additional features or bug fixes, and you can easily get those changes from now on by entering:
...
* Create two postgres databases: `Room8` and `Room8-test` (you can substitute these with the name of your own application - just be sure to go through and change the `package.json` and `.travis.yml` to refer to the new name)
  * By default, running `npm test` will use `Room8-test`, while regular development uses `Room8`
...
Some developers may prefer to control deployment rather than rely on automation. Your local copy of the application can be pushed up to Heroku at will, using Room8's handy deployment script:
